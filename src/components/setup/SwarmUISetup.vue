...
const hostInput = ref(store.settings.swarmui_host || 'http://localhost:7801');
...
async function testConnection() {
  ...
  try {
    const response = await fetch(`${hostInput.value}/API/GetNewSession`, {
      method: 'POST',
      body: '{}',
      headers: {
    ...
    features.value = { txt2img: true };
    version.value = data.version;

    store.settings.selected_provider_image = 'swarmui';
    store.settings.swarmui_host = hostInput.value;
...
<template>
  <div class="space-y-4">
    <div class="space-y-2">
      <label class="text-sm font-medium">SwarmUI Host</label>
      <div class="flex gap-2">
        <Input v-model="hostInput" placeholder="http://localhost:7801" :class="{
    ...
    <div v-if="connectionStatus !== 'none'" class="text-sm">
      ...
      <div v-if="connectionStatus === 'success'" class="text-green-400">
        <div class="font-medium inline-block"> Connected successfully! <span class="text-gray-600 ml-2">SwarmUI v{{
          version }}</span>
        </div>
        <div>
